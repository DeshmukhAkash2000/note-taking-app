...
const noteReducer = (state, action) =>{
    switch (action.type) {
        case "SAVE":
            return(action.payload.label !== "" ? {state, saveLable : [...state.saveLable, action.payload], saveNote: [...state.saveNote, action.payload]} : {...state, saveNote :[...state.saveNote, action.payload ]} );
        ...
        case "REMOVE_FROM_ARCHIEVE":
            const updateArchieve = state.saveArchieve.filter(item => item.id !== action.payload.id);
